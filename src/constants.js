export const scaleFactor = 4;

export const dialogueData = {
    pc: `This is my PC. I work mostly in JavaScript, Python and Java these days.
      I've experience using these languages at a professional level. Anyway regardless of the language, I just like programming.`,
    "cs-degree": `This is my CS Master's degree from University of Illinois at Urbana Champaign (UIUC). 
    I hung it on the wall because I'm very proud of it. It was a very practical degree and gave me a good foundation through courses such as Database Systems, Software Engineering, AI, Cloud Computing, Computational Photography, Machine Learning for SE, and Advanced Data Management.`,
    "sofa-table": `That's my sofa. I like to relax here and watch YouTube for latest tech news, geopolitical discussions and learn about new places to tour and travel (Japan being my next dream destination !) `,
    tv: `That's my TV. I've been watching scifi shows a lot recently like :
     <a href="https://www.netflix.com/title/80097140" target="_blank">Altered Carbon</a>, <a href="https://www.amazon.com/The-Expanse-Season-1/dp/B08B48L4CQ" target="_blank">The Expanse</a>,
    <a href="https://www.netflix.com/title/81024821" target="_blank">3 Body Problem</a> and <a href="https://tv.apple.com/us/show/for-all-mankind/umc.cmc.6wsi780sz5tdbqcf11k76mkp7" target="_blank">For All Mankind</a>!`,
    bed: `This where I decompress and rest. Great ideas comes when I'm lying on my bed. When an idea strikes, I immediately write it down or else I won't be able to sleep because my mental energy is consumed by it.`,
    resume: `This is my desk and on it is my resume. <a href="https://copper-dominica-62.tiiny.site/" target="_blank">Check it out?</a>
    Contact me at lakshya41997@gmail.com if you have any interesting job opportunities!`,
    projects: `Info about this application : It's made with the Kaboom.js library which is a library for making games in JavaScript.
    Text is rendered with HTML/CSS. So the textbox you're currently reading is not rendered within canvas.`,
    library: `There are a lot of programming books on my shelves.
    I probably only read one of them. Who else compulsively buys technical books without ever finishing them?`,
    exit: `If you want to exit Lakshya's portfolio, just close the tab.`,
  };